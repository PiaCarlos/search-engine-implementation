...
public class MyWebGraph {
	...
	public boolean addVertex(String s) {
		...
		// if the url is already part of the graph. Do nothing
		if (this.vertexList.containsKey(s)) { 
			return false;
		}
	...
	public boolean addEdge(String s, String t) {
		...
		// if the urls are not already part of the graph. Do nothing
		if(!this.vertexList.containsKey(s) || !this.vertexList.containsKey(t)) {
			return false;
		}
		
		WebVertex firstVector = this.vertexList.get(s);
		return firstVector.addEdge(t);
	}
    ...
    public ArrayList<String> getVertices() {
    	...
    	// adding all the urls in the array.
    ...
    public ArrayList<String> getEdgesInto(String v) {
    	...
    	// WebVertex v_Vertex = this.vertexList.get(v);
    	
    	// add to allpages, if the vertex as an edge to v
    	for (String url : vertexList.keySet()) {
    		WebVertex urlVertex = this.vertexList.get(url);
    		if(urlVertex.containsEdge(v)) {
    			allPages.add(url);
    		}
    	}
